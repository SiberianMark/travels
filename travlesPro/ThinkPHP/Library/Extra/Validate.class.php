...
class Validate
{
 ...
 // 指定字体颜色
    ...
    {
        $this->font = dirname(__FILE__) . '/font/consola.ttf'; // 注意字体路径要写对，否则显示不了图片
    }
    // 生成随机码
    ...
    private function createFont ()
    {
        ...
        for ($i = 0; $i < $this->codelen; $i ++) {
            $this->fontcolor = imagecolorallocate($this->img, mt_rand(0, 156),
                    mt_rand(0, 156), mt_rand(0, 156));
            imagettftext($this->img, $this->fontsize, mt_rand(- 30, 30),
                    $_x * $i + mt_rand(1, 5), $this->height / 1.4,
    ...
    private function createLine ()
    {
        ...
        for ($i = 0; $i < 10; $i ++) {
            ...
            imagestring($this->img, mt_rand(1, 5), mt_rand(0, $this->width),
                    mt_rand(0, $this->height), '*', $color);
        }
    }
    // 输出
