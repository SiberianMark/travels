...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 添加出游客人

</nav>
<form action="/index.php/Admin/TravelInfo/doadd" method="post">
	<div class="page-container" style="width: 900px;margin:20px auto;">
        ...
        <table class="table table-border table-bordered table-hover mt-10 table1">
			...
			<tr>
				...
				<td colspan="3"><input class="input-text" type="text" name="lineName" value="<?php echo ($travelInfo["lineName"]); ?>"/></td>
	...
	<table id="addVisitors" class="table table-border table-bordered table-hover" style="margin:10px auto 0; <?php if($travelInfo['lineType'] == 4){ echo 'width:1200px';}else{echo 'width:940px';}?>">
		<?php
 if ($member) { $content = ''; if ($travelInfo['lineType'] == 4) { $content .= '<tr><th style="text-align: center;">真实名字</th>'. '<th style="text-align: center;">电话</th>' . '<th style="text-align: center;">身份证号</th>' . '<th style="text-align: center;">上车地点</th>' . '<th style="text-align: center;">出生年月日</th>' . '<th style="text-align: center;">出生地</th>' . '<th style="text-align: center;">签发地</th>' . '<th style="text-align: center;">收客价</th>' . '<th style="text-align: center;">网站结算价</th>' . '<th style="text-align: center;">地接结算价</th></tr>'; foreach ($member as $list) { $content .= '<tr><td style="width: 100px"><input class="input-text" type="text" name="trueName[]" value="'.$list['trueName'].'"/></td>'. '<td style="width: 120px"><input class="input-text" type="text" name="phone[]" value="'.$list['phone'].'"/></td>'. '<td style="width: 180px"><input class="input-text" type="text" name="idNumber[]" placeholder="身份证或护照号，通行证号" value="'.$list['idNumber'].'"/></td>'. '<td><input class="input-text" type="text" name="musterAddr[]" value="'.$list['musterAddr'].'"/></td>'. '<td style="width: 90px"><input class="input-text" type="text" name="birthDate[]" value="'.$list['birthDate'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="birthAddr[]" value="'.$list['birthAddr'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="signAddr[]" value="'.$list['trueName'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="payPrice[]" value="'.$list['payPrice'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="settlementPrice[]" value="'.$list['settlementPrice'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="agencyPrice[]" value="'.$list['agencyPrice'].'"/></td></tr>'; } } else { $content .= '<tr><th style="text-align: center;">真实名字</th>'. '<th style="text-align: center;">电话</th>' . '<th style="text-align: center;">身份证号</th>' . '<th style="text-align: center;">上车地点</th>' . '<th style="text-align: center;">收客价</th>' . '<th style="text-align: center;">网站结算价</th>' . '<th style="text-align: center;">地接结算价</th></tr>'; foreach ($member as $list) { $content .= '<tr><td style="width: 100px"><input class="input-text" type="text" name="trueName[]" value="'.$list['trueName'].'"/></td>'. '<td style="width: 120px"><input class="input-text" type="text" name="phone[]" value="'.$list['phone'].'"/></td>'. '<td style="width: 180px"><input class="input-text" type="text" name="idNumber[]" placeholder="身份证或护照号，通行证号" value="'.$list['idNumber'].'"/></td>'. '<td><input class="input-text" type="text" name="musterAddr[]" value="'.$list['musterAddr'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="payPrice[]" value="'.$list['payPrice'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="settlementPrice[]" value="'.$list['settlementPrice'].'"/></td>'. '<td style="width: 60px"><input class="input-text" type="text" name="agencyPrice[]" value="'.$list['agencyPrice'].'"/></td></tr>'; } } echo $content; } else { } ?>
	</table>
	<div style="width:300px; margin:40px auto 60px">
		<button name="" style="margin-left: 10px; width:110px" class="btn btn-success radius" type="submit">保&nbsp;&nbsp;存</button>
		&nbsp;&nbsp;&nbsp;&nbsp;
