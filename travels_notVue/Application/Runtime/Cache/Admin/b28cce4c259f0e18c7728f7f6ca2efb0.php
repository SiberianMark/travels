...
<article class="page-container">
	<form action="/index.php/Admin/Employee/adminDepaEdit" method="post" class="form form-horizontal" id="form-admin-role-add" target="_parent">
		<input type="hidden" name="id" value="<?php echo ($depa["id"]); ?>"/>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>部门名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($depa["name"]); ?>" placeholder="" id="roleName" name="name" datatype="*4-16" nullmsg="用户账户不能为空">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">部门描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($depa["detail"]); ?>" placeholder="" id="" name="detail">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<?php if(is_array($lists)): foreach($lists as $key=>$list): ?><dl class="permission-list">
						<dt>
							<label>
								<input type="checkbox" value="<?php echo ($list["id"]); ?>" name="rules[]" <?php if(isset($list["checked"])): ?>checked<?php endif; ?> />
								<?php echo ($list["title"]); ?></label>
						</dt>
						<?php if($list["child"] != ''): ?><dd>
								<?php if(is_array($list["child"])): foreach($list["child"] as $key=>$v): ?><label style="padding-right:0.625em;"><input class="menu-child" type="checkbox" value="<?php echo ($v["id"]); ?>" pid="<?php echo ($v["pid"]); ?>" name="rules[]" <?php if(isset($v["checked"])): ?>checked<?php endif; ?>/><?php echo ($v["title"]); ?></label><?php endforeach; endif; ?>
							</dd><?php endif; ?>
